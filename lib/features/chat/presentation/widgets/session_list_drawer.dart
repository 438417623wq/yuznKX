...
import '../screens/group_manager_screen.dart';
...
class _SessionListDrawerState extends ConsumerState<SessionListDrawer> {
  ...
  Widget _buildCharacterList() {
    ...
    final filteredChars =
        characters.where((c) => c.name.toLowerCase().contains(query)).toList();

    if (filteredChars.isEmpty) {
      return const Center(
        child: Text('没有匹配角色', style: TextStyle(color: Colors.white54)),
      );
    }
            ...
            children: [
              Expanded(
                child: ElevatedButton.icon(
                  ...
                  label: const Text("新建群聊 (Group Chat)"),
                  style: ElevatedButton.styleFrom(
    ...
    return Card(
      ...
      child: ListTile(
        ...
        subtitle: Text(
          isGenerating
              ? '正在回复...'
              : '${char.tags.length} Tags · ${char.creator.isEmpty ? '未知作者' : char.creator}',
          style: TextStyle(
